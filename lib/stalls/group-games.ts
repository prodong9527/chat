...
const GROUP_GAME_FALLBACKS: Record<GroupGameSlug, StallResult> = {
  "newcomer-guide": {
    ...
    sections: [
      ...
      { label: "隐藏条例", value: "“我来同步一下”意为我们一起把事情讲明白。" },
    ],
    shareTemplate: "handbook",
    isFallback: true,
  },
...
const SectionSchema = z.object({ label: z.string().min(1).max(30), value: z.string().min(1).max(240) }).strict();

function resultSchema(template: GroupGameDefinition["shareTemplate"], labels: readonly string[]) {
  return z.object({
    title: z.string().min(1).max(80),
    summary: z.string().min(1).max(240),
    sections: z.array(SectionSchema).length(labels.length).superRefine((sections, context) => {
      sections.forEach((section, index) => {
        if (section.label !== labels[index]) context.addIssue({ code: "custom", path: [index, "label"], message: "栏目不符合该摊位回执格式" });
      });
    }),
    shareTemplate: z.literal(template),
    isFallback: z.literal(false).optional(),
  }).strict();
}

const GroupGameResultSchemas = {
  "newcomer-guide": resultSchema("handbook", ["入职首日必修课", "生存装备", "直属领导饲养指南", "隐藏条例"]),
  "meeting-exit": resultSchema("drill", ["突发事件", "当前逃生身份", "三步逃生动作", "预计成功率"]),
  "performance-defense": resultSchema("award", ["奖项名称", "表彰事由", "可量化的虚构成果", "评审委员会批语"]),
} as const;
...
export function parseGroupGameResult(slug: GroupGameSlug, result: StallResult): StallResult {
  return GroupGameResultSchemas[slug].parse(result);
}
