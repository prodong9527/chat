...
Updated `lib/market/types.test.ts` so the test uses a complete valid `StallSchema` fixture, verifies that fixture parses unchanged, removes only its `slug`, and verifies that the otherwise valid stall is rejected.
...
```text
npm test -- lib/market/types.test.ts && npm test
```

Output:

```text
Focused: 1 test passed in 1 test file.
Full suite: 1 test passed in 1 test file.
