...
vi.mock("@/lib/market/metrics", () => ({
  recordMetricBestEffort: vi.fn(async () => true),
}));
...
describe("metrics API", () => {
  ...
  it("records one generation for every successful acceptance", async () => {
    const response = await POST(new Request("https://9527.example/api/metrics", {
      method: "POST", headers: { "content-type": "application/json" },
    ...
    expect(response.status).toBe(200);
    expect(recordMetricBestEffort).toHaveBeenCalledWith("huafu-badge", "generation");
  });
});
