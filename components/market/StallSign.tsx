...
export function StallSign({ stall }: { stall: PublicStall }) {
  ...
  return <Link href={href} className={`stall-sign ${stall.status !== "open" ? "stall-closed" : ""}`}>
    <span className="stall-code">{stall.code}</span>{stall.hotRank && <em className="hot-stall">热门 #{stall.hotRank}</em>}<h3>{stall.name}</h3><p>{stall.description}</p>
    <footer><span>{stall.status === "open" ? "正在受理" : "即将开张"}</span><b><MetricCount slug={stall.slug} initial={stall.generations} /><small>累计受理 {stall.totalGenerations} 件</small></b></footer>
  </Link>;
}
