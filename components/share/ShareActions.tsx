...
export function ShareActions({ template, payload, filename, onSaved }: { template: ShareTemplate; payload: SharePayload; filename: string; onSaved?: () => void }) {
  ...
  useEffect(() => { if (canvasRef.current) drawShareCard(canvasRef.current, { ...payload, footer: payload.footer ?? `华府 ${template} 回执` }, selectShareSize(window.innerWidth)); }, [payload, template]);
